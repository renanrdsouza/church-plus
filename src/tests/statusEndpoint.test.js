test("should return database status info and http status code 200", async () => {
  ...
  const databaseMaxConnections = responseBody.dependencies.database.max_connections;
  const databaseActiveConnections = responseBody.dependencies.database.opennedConnections;
  const parsedUpdatedAt = new Date(responseBody.updated_at).toISOString();
  
  expect(response.status).toBe(200);
  expect(responseBody.updated_at).toBeDefined();
  expect(responseBody.updated_at).toEqual(parsedUpdatedAt);
  expect(databaseVersion).toBe('16.2');
